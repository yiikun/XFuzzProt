...
Emu::Emu(int argc, char **argv) {
    ...
    Cycles = 0;
    dut_ptr = new VTestTop_L2L3L2();  // scala test interface from CoupledL2/src/test/scala/TestTop.scala
...
void Emu::execute(uint64_t nr_cycle) 
{
    while (Cycles < nr_cycle) 
    {
        ...
        {
            agents[i]->handle_channel();  // fire channel a, b, c, d, e for every agent
        }

        for (int i = 0; i < NR_AGENTS; i++) 
        {
            fuzzers[i]->tick();    // CFuzzer::randomTest: do_acquireBlock, do_acquirePerm, do_releaseDataAuto  ==>  make a pending
        }

        for (int i = 0; i < NR_AGENTS; i++) 
        {
            agents[i]->update_signal();  // send by pending, record in localboard and put into ports 
        }
...
tl_agent::Port<tl_agent::ReqField, tl_agent::RespField, tl_agent::EchoField, BEATSIZE>* Emu::naive_gen_port() {
    ...
    port->a.mask = (uint32_t*)&(dut_ptr->master_port_0_0_a_bits_mask); // @makun: 
    ...
    port->b.alias = (uint8_t *)&(dut_ptr->master_port_0_0_b_bits_data); // @makun
...
tl_agent::Port<tl_agent::ReqField, tl_agent::RespField, tl_agent::EchoField, BEATSIZE>* Emu::naive_gen_port2() {
    ...
    port->a.mask = (uint32_t*)&(dut_ptr->master_port_1_0_a_bits_mask); // @makun: 
    ...
    port->b.alias = (uint8_t*)&(dut_ptr->master_port_1_0_b_bits_data); // @makun
