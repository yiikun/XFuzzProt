...
namespace tl_agent {
    ...
    Resp CAgent::send_a(std::shared_ptr<ChnA<ReqField, EchoField, DATASIZE>> &a) {
        switch (*a->opcode) {
            case AcquireBlock: {
                ...
                idMap->update(*a->source, idmap_entry);    // id --> (address, alias)

                if (localBoard->haskey(*a->address)) {    // *(a->address)
                    localBoard->query(*a->address)->update_status(S_SENDING_A, *cycles, *a->alias);  // (a->address) --> (SBEntry.status = S_SENDING_A)
                } else {
    ...
    bool CAgent::do_acquirePerm(paddr_t address, int param, int alias) {  // Permission
        if (pendingA.is_pending() || pendingB.is_pending() || idpool.full())
            return false;
